defmodule Hangman.Game do
    ...
    def make_move(game = %{ game_state: state }, _quess) when state in [:won, :lost] do
        { game, tally(game) }
    end

    # def make_move(game = %{ game_state: :lost }, _quess) do
    #     { game, tally(game) }
    # end
    ...
    def reveal_guessed(letters, used) do
        letters
        |> Enum.map(fn letter -> reveal_letters(letters, MapSet.member?(used, letter)) end )
    end

    def reveal_letters(letter, _in_word = true), do: letter
    def reveal_letters(letter, _not_in_word), do: "_"
